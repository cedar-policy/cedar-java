...
sed "87s;.*;$ffi_lib_str;" "build.gradle" > new_build.gradle
...
if [ "$#" -ne 0 ] && [ "$1" == "run_int_tests" ]; then
    ...
    sed "86s;.*;$integration_tests_str;" "build.gradle" > new_build.gradle
    mv new_build.gradle build.gradle

    export MUST_RUN_CEDAR_INTEGRATION_TESTS=1

    cargo_str='cedar-policy = { version = "2.5", path = "../cedar/cedar-policy" }'
